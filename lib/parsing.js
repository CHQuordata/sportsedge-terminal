...
export function extractNBAStats(d) {
  ...
  const oppPpg = dv('avgPointsAllowed', 'oppPointsPerGame', 'avgPointsAgainst');
  const netPts = ppg != null && oppPpg != null
    ? (parseFloat(ppg) - parseFloat(oppPpg)).toFixed(1)
    : null;
  // ESPN returns pace as 'paceFactor' in the site stats endpoint
  const paceVal = nv('paceFactor', 'pace', 'avgPossessions', 'possessionsPerGame');
