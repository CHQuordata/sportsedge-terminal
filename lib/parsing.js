...
export function extractNBAStats(d) {
  if (!d) return null;
  const all = [];
  const cats = d.results?.stats?.categories || d.results?.splits?.categories || d.splits?.categories || [];
  ...
  const nv = n => { const s = all.find(x => x.name === n); return s?.value ?? null; };
  const ppg = dv('avgPoints'), oppPpg = dv('avgPointsAllowed');
  const netPts = (ppg && oppPpg) ? (parseFloat(ppg) - parseFloat(oppPpg)).toFixed(1) : null;
  const paceVal = nv('pace');
  const pace = paceVal != null
    ? parseFloat(paceVal.toFixed(1))
    : (dv('pace') ? parseFloat(dv('pace')) : null);
  return {
    ppg, fgPct: dv('fieldGoalPct'), tpPct: dv('threePointPct'), ftPct: dv('freeThrowPct'),
